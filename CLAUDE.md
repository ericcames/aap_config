...
- **Repo-shipped skills** live in `.claude/skills/` and mirror the runbooks. The
  authoritative list is [`AGENTS.md` → Repo-shipped
  skills](AGENTS.md#repo-shipped-skills) — don't duplicate it here. They are
  **not Claude-only**: Copilot CLI reads `.claude/skills/` as a project skills
  directory, so keep every skill tool-neutral and keep the frontmatter to
  `name` + `description`. Prefer following a skill over improvising — it keeps
  you aligned with what a trainee is taught to do. Skills reference
  `docs/runbooks/` rather than duplicating steps. When making a design change
  (new convention, architecture decision), use `/evolve-kit` for the full
  research → plan → implement → PR cycle.
